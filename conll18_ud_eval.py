...
def load_conllu(file, discarded=[]):
    ...
    class UDWord:
        def __init__(self, span, columns, is_multiword):
            ...
            #print("Ignore subtypes.")
            #self.columns[DEPREL] = columns[DEPREL].split(":")[0]
            # DO NOT ignore language-specific deprel subtypes.
            self.columns[DEPREL] = columns[DEPREL]
            # Precompute which deprels are CONTENT_DEPRELS and which FUNCTIONAL_DEPRELS
            self.is_content_deprel = columns[DEPREL] in CONTENT_DEPRELS
            self.is_functional_deprel = columns[DEPREL] in FUNCTIONAL_DEPRELS

    ud = UDRepresentation()

    global cycle_count
    cycle_count, multi_roots_count = 0, 0
        ...
        if not line:
            ...
            def process_word(word):
                if word.parent == "remapping":
                    ...
                    global cycle_count
                    cycle_count += 1
                    #print(f"{cycle_count} - There is a cycle in a sentence")
            ...
            if len([word for word in ud.words[sentence_start:] if word.parent is None]) != 1:
                ...
                #global multi_roots_count
                multi_roots_count += 1
                #print(f"{multi_roots_count} - There are multiple roots in a sentence")
    ...
    #global cycle_count, multi_roots_count
    return ud, cycle_count, multi_roots_count
...
def evaluate(gold_ud, system_ud):
    ...
    if gold_ud.characters != system_ud.characters:
        index = 0
        while index < len(gold_ud.characters) and index < len(system_ud.characters):
            gold_c = _encode(gold_ud.characters[index])
            system_c = _encode(system_ud.characters[index])
            if re.match(r"[A-Za-z]", gold_c) and re.match(r"[A-Za-z]", system_c) and gold_c.lower() != system_c.lower():
                break
            else:
                index += 1
                
        if index < min([len(system_ud.characters), len(gold_ud.characters)]):
            raise UDError(
...
import time
def load_conllu_file(path, discarded=[]):
    print(path)
    if len(discarded) > 0:
        with open(path, 'r') as f:
            sents = f.read().split('\n\n')

        sents = [s for i, s in enumerate(sents) if i not in discarded]
        tmp = f'cache/remained_{time.time()}.conllu'
        with open(tmp, 'w') as f:
            f.write("\n\n".join(sents))
    else:
        tmp = path
    _file = open(tmp, mode="r", **({"encoding": "utf-8"} if sys.version_info >= (3, 0) else {}))
